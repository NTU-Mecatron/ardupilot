...
> This is the `Rover-4.5` branch. For `Sub-4.5` and `Copter-4.5`, please refer to their respective branches.
...
```bash
cd ~/ardupilot
git clone --recursive -b Rover-4.5 https://github.com/NTU-Mecatron/ardupilot.git rover-4.5
cd rover-4.5
```
...
docker run --rm -it -v $PWD:/ardupilot ardupilot-dev ./waf rover
```
...
```bash
docker run --rm -it --privileged -v $PWD:/ardupilot ardupilot-dev ./waf --upload-port="/dev/ttyACM0" --upload rover
```
...
To change one or two parameters, you can use:

```bash
param set SOME_PARAMETER 0
reboot
```
...
To make a differential-thrust USV work properly in UnityMDS, you must upload the below parameter file:

```bash
param load params/differential_boat.parm
reboot
```
